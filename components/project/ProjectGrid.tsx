...
export function ProjectGrid({
  projects, loading, error, total, page, totalPages, onPageChange,
}: ProjectGridProps) {
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <button onClick={() => window.location.reload()} className="btn btn-secondary mt-1">
          Try again
        </button>
      </div>
    );
    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {loading
          ...
          : projects.map((p, i) => (
              <ProjectCard key={p._id} project={p} priority={i < 3} />
            ))}
      </div>
        ...
        p === "..." ? (
          ...
          <button
            ...
            className={cn(
              ...
              p === page ? "text-white shadow-sm" : "btn btn-secondary"
            )}
